...
3. 绑定执行工具（`createForgeTools`：仅 `exec_shell` / `git_op`）+ `filesystem: true`（Composite `/workspace/`）。
...
1. **文件统一 Composite，shell 仍隔离**：主 Agent / Forge implement·fix 共用 `createFilesystemMiddleware` + Composite（`/workspace/` → FilesystemBackend，`@chatvein/agents`）。`createForgeTools` 仅剩 `exec_shell` / `git_op`（`SandboxProvider` 白名单）。详见 `.agents/notes/2026-09-07-deepagents-filesystem-composite.md`。
