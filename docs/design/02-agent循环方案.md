...
> 版本：v0.3 ｜ 日期：2026-09-04
> 上位：[`01-核心骨架.md`](./01-核心骨架.md) ｜ 记忆：[`03-记忆方案.md`](./03-记忆方案.md)
> Chat 执行层专文（L3）：[`11-L3-ReAct自适应循环推理层.md`](./11-L3-ReAct自适应循环推理层.md)
...
> 变更：v0.3 区分 **用户拉群** vs **Agent 派生子 Agent**
...
- **单 Agent**（普通对话 / 群里某个成员发言）：模型自主决定工具调用；复杂时可在同一会话内**派生子 Agent**（仍属 agents 运行时，不是拉群）。
- **多 Agent · 群组协作**：前提是**用户已在客户端建群**；在单 Agent 循环之上加一层「**发言路由 / 编排**」，决定下一个说话的是谁。

> **拉群 ≠ 子 Agent**：拉群 / 建群 / 升级成群由用户操控客户端完成；Agent 自发的多智能体形态是创建子 Agent。见 [01 §5.0](./01-核心骨架.md)、[09](./09-启发式规则路由.md)。
...
扩展（P0+）：在 ReAct 之上按策略 **创建子 Agent**（分视角/子任务），结果汇总回父循环——**不**因此创建 Group。
...
适用：开放式讨论、方案评审、多视角辩论（**群须由用户先建好**）。优点：灵活、类企业微信直觉。缺点：容易跑题/话痨，需主持人与轮次/预算约束。
