...
| 自动 | `playwright` | 目录选中 `mcp_playwright` 且未 `mcpPlaywright:false`（浏览器自动化；**不依赖** workspace） |
...
- playwright `args` = `[@playwright/mcp cli.js, --headless]`（可选 `--browser=`）
...
| `playwright__*` | MCP `playwright`（`@playwright/mcp` 浏览器自动化） |
