...
| PDF 处理 | `mcp-pdf` | `pdf_info` `pdf_read` `pdf_merge` `pdf_split` `pdf_generate` `pdf_encrypt` `pdf_decrypt` | 读取用 `pypdf`（文本层、页数/元信息、加密状态），生成用 `reportlab`（内置中文字体，不依赖字体文件）。页数上限 400、文本截断 `max_chars`；扫描件（无文本层）返回提示并建议配合 `ocr_image`。路径规则同 `mcp-fs` |
