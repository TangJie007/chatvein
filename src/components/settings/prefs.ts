...
export const MCP_SERVERS: McpServer[] = [
  {
    ...
    kind: "db",
    builtin: true,
    desc: "读写本机 SQLite 数据库，执行查询并回传结构化结果，Agent 的数据落点。",
    transport: "本机进程",
  ...
  },
  {
    ...
    kind: "fs",
    builtin: true,
    desc: "受限目录内的读写、检索与批量重命名，越界路径由护栏二次确认。",
    transport: "本机进程",
  ...
  },
  {
    ...
    kind: "web",
    builtin: true,
    desc: "网页检索与正文抽取，为知识问答补充实时信息。",
    transport: "HTTP",
    cmd: "builtin://mcp-web --endpoint /internal/search",
    tools: 4,
    enabled: true,
  },
  {
    ...
    kind: "kb",
    builtin: true,
    desc: "向量召回 + 重排（sqlite-vec），命中片段直接注入上下文，供多 Agent 共享。",
    transport: "本机进程",
    cmd: "builtin://mcp-kb --index ./data/vectors",
    tools: 3,
    enabled: true,
  },
  {
    ...
    kind: "custom",
    builtin: true,
    desc: "接入任意 MCP 服务，填写地址后自动拉取工具清单。",
    transport: "HTTP",
    cmd: "尚未配置服务地址",
    tools: 0,
    enabled: false,
