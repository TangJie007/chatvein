/**
 * L2 模型调用：withStructuredOutput 主路径 → 纯文本 JSON 兜底。
 * 失败不重试，返回 null 由编排层标 unavailable。
 */
import type { LanguageModelLike } from '@langchain/core/language_models/base'
import type { BaseMessage } from '@langchain/core/messages'
import { contentToString, extractJsonObject, withTimeoutSignal } from '../../shared'
import { L2RawSchema, parseL2Judgement } from './schema'
import type { L2Judgement } from './types'

export interface InvokeL2Options {
...
function callOptions(opts: InvokeL2Options): Record<string, unknown> {
 ...
 */
export async function invokeL2(options: InvokeL2Options): Promise<L2Judgement | null> {
  ...
  if (typeof withStructured === 'function') {
    try {
      const extractor = (
        withStructured as (
          schema: typeof L2RawSchema,
          opts?: { name: string },
        ) => StructuredInvoker
      ).call(model, L2RawSchema, { name: 'router_l2' })
      return parseL2Judgement(await extractor.invoke(messages, invokeOpts))
    } catch {
  ...
  try {
    ...
    return parseL2Judgement(extractJsonObject(contentToString(res.content)))
  } catch {
    return null
  }
}
